...
async function restoreSession() {
  // Session persistence is disabled for room comparison.
  return;
}
...
function renderLatestRoomRecommendations(room) {
  if (!recommendationsGrid || !recommendationsEmpty) return;
  if (!room || !Array.isArray(room.recommendations) || !room.recommendations.length) {
    recommendationsGrid.innerHTML = '';
    recommendationsEmpty.style.display = 'flex';
    return;
  }
  ...
  recommendationsEmpty.style.display = 'none';
  console.log('rendering recommendations:', room.recommendations);
  recommendationsGrid.innerHTML = room.recommendations.map((recommendation, index) => `
...
function showLatestResult() {
  ...
  renderSavedRoomCards();
  renderLatestRoomRecommendations(room);
...
async function handleSubmit(e) {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/api/rooms/analyze`, {
      method: 'POST',
    ...
    if (!res.ok) {
      ...
      throw new Error(`Room analyze API returned ${res.status}: ${errorBody}`);
    }

    const json = await res.json();
    room = json?.data?.room || null;
    usedApi = !!room;
  } catch (err) {
    handleRoomApiError(err, 'Room analyze');
  }
...
function initializeComparePage() {
  if (!roomForm) {
    console.error('Room Comparison page initialization failed: room form not found.');
    return;
  }
  ...
  if (compareBtn) {
    compareBtn.addEventListener('click', async () => {
      ...
      let usedApi = false;
      try {
        const res = await fetch(`${API_BASE_URL}/api/rooms/analyze-batch`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (!res.ok) {
          const errorBody = await res.text();
          throw new Error(`Room batch API returned ${res.status}: ${errorBody}`);
        }
        ...
        handleRoomApiError(err, 'Batch analyze');
      }
...
window.addEventListener('DOMContentLoaded', initializeComparePage);
