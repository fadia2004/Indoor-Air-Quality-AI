...
function speakRecommendations() {
  ...
  if (window.speechSynthesis.speaking) {
    window.speechSynthesis.cancel();
    readRecommendationsBtn.textContent = 'Read Recommendations';
    return;
  }
  ...
  readRecommendationsBtn.textContent = 'Stop Reading';
  window.speechSynthesis.speak(utterance);
}
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/api/quick-scan/predict`, {
      method: 'POST',
    ...
    if (!res.ok) {
      ...
      throw new Error(`Quick Scan API returned ${res.status}: ${errorBody}`);
    }
  ...
  if (!usedApi) {
    voiceStatus.textContent = 'Backend unavailable — using local prediction.';
    computePrediction();
  }
...
function handleApiError(error) {
  console.error('Quick Scan API failed:', error);
  voiceStatus.textContent = 'Prediction backend unavailable.';
}
